...
export const gradientPlugin: Plugin<"pie" | "doughnut"> = {
  ...
  beforeDraw(chart) {
    ...
    const secondaryGradientColors = gradientColors.secondary as [
      string,
      string,
    ];
    ...
    const datasetPrimary = chart.data.datasets.find(
      (dataset) => dataset.label === CHART_SETTINGS.DATA_SET_NAME,
    );
...
export const backgroundPlugin: Plugin<"pie" | "doughnut"> = {
  ...
  afterDatasetsDraw(chart, _, options) {
    ...
    const innerRadius =
      backgroundOptions.innerRadius ?? CHART_SETTINGS.INNER_RADIUS_GENERAL + 1;
    const innerColor =
      backgroundOptions.innerColor ?? CHART_SETTINGS.GREY_BACKGROUND_COLOR;
