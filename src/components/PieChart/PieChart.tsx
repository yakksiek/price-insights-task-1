...
import {
  IChartData,
  IGradientColorsOptions,
  IShadowColorsOptions,
  IThicknessPluginOptions,
} from "./types";
import {
  thicknessPlugin,
  innerTextPlugin,
...
export const PieChart: FC<IProps> = ({
  ...
  const primaryBorderColor = coveredState
    ? CHART_SETTINGS.GREY_BACKGROUND_COLOR
    : "";
  const primaryBorderThickness = coveredState ? 1 : 0;
  const secondarySliceColor = notCoveredState ? secondaryColor : "transparent";
  const secondaryBorderColor = notCoveredState
    ? CHART_SETTINGS.GREY_BACKGROUND_COLOR
    : "";
