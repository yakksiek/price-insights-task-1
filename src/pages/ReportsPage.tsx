  ...
  const handleDrop = useCallback(
    (args: BaseEventPayload<ElementDragType>) => {
      ...
      if (draggableSourceType === "card") {
        ...
        const sourceBlockData = blocks.find(
          (block) => block.id === sourceBlockId,
        );
        if (!sourceBlockData) return;

        const draggedCardIndex = u.returnItemIndexInArrById(
          draggedCardId,
          sourceBlockData.items,
        );

        // check if source block is the same as destination block
        // because items don't move between blocks
        const [destinationCardRecord, destinationBlockRecord] =
        ...
        const indexOfTarget = u.returnItemIndexInArrById(
          destinationCardId,
          sourceBlockData.items,
        );
      ...
      if (draggableSourceType === "block") {
        ...
        const destinationBlockIndex = blocks.findIndex(
          (block) => block.id === destinationBlockId,
        );

        // Determine the closest edge of the target card: top or bottom
        const closestEdgeOfTarget = extractClosestEdge(
          destinationBlockRecord.data,
        );
        ...
        const reorderedBlocks = u.reorder(
          blocks,
          draggedBlockIndex,
          destinationIndex,
        );
